  ...
  return (
    <div className="px-8 py-6 bg-[#0d0f12] min-h-screen">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 text-sm text-[#a0a0c0] mb-2">
            <Link href="/sites" className="hover:text-[#eef0f6]">Sites</Link>
            <span className="text-[#252a38]">/</span>
            <span className="text-[#eef0f6] font-medium">{site.name}</span>
          </div>
          <h1 className="text-2xl font-bold text-[#eef0f6]">{site.name}</h1>
          {latestScan && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="rounded-lg bg-[#141720] border border-[#252a38] p-5 flex flex-col items-center justify-center">
          <div className="text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider mb-2">Priority</div>
          {worstTier ? (
            <>
        ...
        <div className="rounded-lg bg-[#141720] border border-[#252a38] p-5">
          <div className="text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider mb-2">WCAG Errors</div>
          <div className="text-3xl font-bold text-[#eef0f6] tabular-nums">{latestScan?.raw_violation_count ?? '—'}</div>
          <div className="text-xs text-[#a0a0c0] mt-1">{latestScan?.unique_pattern_count ?? 0} issue types</div>
        </div>

        <div className="rounded-lg bg-[#141720] border border-[#252a38] p-5">
          <div className="text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider mb-2">Pages Scanned</div>
          <div className="text-3xl font-bold text-[#eef0f6] tabular-nums">{latestScan?.pages_scanned ?? '—'}</div>
          <div className="text-xs text-[#a0a0c0] mt-1">{pages.length} configured</div>
        </div>

        <div className="rounded-lg bg-[#141720] border border-[#252a38] p-5">
          <div className="text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider mb-2">Total Scans</div>
          <div className="text-3xl font-bold text-[#eef0f6] tabular-nums">{scans.length}</div>
          <div className="text-xs text-[#a0a0c0] mt-1">{completedScans.length} completed</div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {/* Violations */}
        <div>
          <h2 className="text-lg font-semibold text-[#eef0f6] mb-4">WCAG Errors</h2>
          {patterns.length === 0 ? (
            <div className="bg-[#141720] rounded-xl border border-dashed border-[#252a38] p-10 text-center text-[#a0a0c0]">
        ...
        {/* Scan History */}
        <div>
          <h2 className="text-lg font-semibold text-[#eef0f6] mb-4">Scan History</h2>
          <div className="rounded-lg bg-[#141720] border border-[#252a38] overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-[#1e2230] border-b border-[#252a38]">
                <tr>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Started</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Status</th>
                  <th className="text-right px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Pages</th>
                  <th className="text-right px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Issues</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Triggered By</th>
              ...
              <tbody className="divide-y divide-[#252a38]">
                {scans.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="text-center py-8 text-[#a0a0c0]">No scans yet.</td>
                  </tr>
                ) : (
                  scans.map((scan: any) => (
                    <tr key={scan.id} className="hover:bg-[#1e2230] group relative cursor-pointer transition-colors">
                      ...
                      <td className="px-4 py-3 text-right text-[#eef0f6]">{scan.pages_scanned ?? 0}</td>
                      <td className="px-4 py-3 text-right text-[#eef0f6]">{scan.raw_violation_count ?? 0}</td>
                      <td className="px-4 py-3 text-[#a0a0c0] capitalize">{scan.triggered_by}</td>
        ...
        {/* Pages */}
        <div>
          <h2 className="text-lg font-semibold text-[#eef0f6] mb-4">Configured Pages</h2>
          {pages.length === 0 ? (
            <div className="text-[#a0a0c0] italic text-sm">No pages configured.</div>
          ) : (
            <div className="rounded-lg bg-[#141720] border border-[#252a38] overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-[#1e2230] border-b border-[#252a38]">
                  <tr>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Label</th>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">URL</th>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#a0a0c0] uppercase tracking-wider">Template Type</th>
                  ...
                  {pages.map((page, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-[#1e2230] transition-colors">
                        <td className="px-4 py-3 font-medium text-[#eef0f6]">{page.label}</td>
                        <td className="px-4 py-3">
                          <PageViolationsModal
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#252a38] text-[#a0a0c0] capitalize">
                            {page.templateType}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right text-[#eef0f6]">
                          {ps ? (ps.violationCount ?? '—') : '—'}
                        </td>
                        <td className="px-4 py-3 text-right">
                          {!ps
                            ? <span className="text-xs text-[#9090b0]">—</span>
                            : ps.score == null
