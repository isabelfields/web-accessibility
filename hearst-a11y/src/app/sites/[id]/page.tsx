  ...
  return (
    <div className="px-8 py-6 bg-[#F5F5F7] min-h-screen">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 text-sm text-[#3A3A3C] mb-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="rounded-lg bg-white border border-[#E5E5EA] p-5 flex flex-col items-center justify-center">
          <div className="text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider mb-2">Priority</div>
          {worstTier ? (
            <>
        ...
        <div className="rounded-lg bg-white border border-[#E5E5EA] p-5">
          <div className="text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider mb-2">WCAG Errors</div>
          <div className="text-3xl font-bold text-[#1D1D1F] tabular-nums">{latestScan?.raw_violation_count ?? '—'}</div>
          <div className="text-xs text-[#3A3A3C] mt-1">{latestScan?.unique_pattern_count ?? 0} issue types</div>
        </div>

        <div className="rounded-lg bg-white border border-[#E5E5EA] p-5">
          <div className="text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider mb-2">Pages Scanned</div>
          <div className="text-3xl font-bold text-[#1D1D1F] tabular-nums">{latestScan?.pages_scanned ?? '—'}</div>
          <div className="text-xs text-[#3A3A3C] mt-1">{pages.length} configured</div>
        </div>

        <div className="rounded-lg bg-white border border-[#E5E5EA] p-5">
          <div className="text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider mb-2">Total Scans</div>
          <div className="text-3xl font-bold text-[#1D1D1F] tabular-nums">{scans.length}</div>
          <div className="text-xs text-[#3A3A3C] mt-1">{completedScans.length} completed</div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          {patterns.length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-[#E5E5EA] p-10 text-center text-[#3A3A3C]">
          ...
          <div className="rounded-lg bg-white border border-[#E5E5EA] overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-[#F5F5F7] border-b border-[#E5E5EA]">
                <tr>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Started</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Status</th>
                  <th className="text-right px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Pages</th>
                  <th className="text-right px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Issues</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Triggered By</th>
              ...
              <tbody className="divide-y divide-[#F0F0F0]">
                {scans.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="text-center py-8 text-[#3A3A3C]">No scans yet.</td>
                  </tr>
                ) : (
                  scans.map((scan: any) => (
                    <tr key={scan.id} className="hover:bg-[#F5F5F7] group relative cursor-pointer transition-colors">
                      ...
                      <td className="px-4 py-3 text-[#3A3A3C] capitalize">{scan.triggered_by}</td>
          ...
          {pages.length === 0 ? (
            ...
            <div className="rounded-lg bg-white border border-[#E5E5EA] overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-[#F5F5F7] border-b border-[#E5E5EA]">
                  <tr>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Label</th>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">URL</th>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#3A3A3C] uppercase tracking-wider">Template Type</th>
                  ...
                  {pages.map((page, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-[#F5F5F7] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#252a38] text-[#3A3A3C] capitalize">
                            {page.templateType}
                          </span>
                        </td>
