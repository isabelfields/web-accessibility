  ...
  return (
    <div className="px-8 py-6 bg-[#0d0f12] min-h-screen">
      ...
      <div className="flex items-center gap-2 text-sm text-[#a0a0c0] mb-6">
        ...
        {site && (
          <>
            <Link href={`/sites/${site.id}`} className="hover:text-[#eef0f6]">{site.name}</Link>
            <span className="text-[#252a38]">/</span>
          </>
        )}
        <span className="text-[#eef0f6] font-medium">Scan {formatDate(scan.started_at)}</span>
      </div>
      ...
      {scan.status !== 'complete' ? (
        ...
        <>
          ...
          {pageScores.length > 0 && (
            <div className="mb-8">
              <h2 className="text-lg font-semibold text-[#eef0f6] mb-4">Page Issues</h2>
                    ...
                    {pageScores.map((ps, i) => (
                      <PageViolationsModal key={i} pageScore={ps} patterns={patterns}>
                        <tr className={`border-t border-[#252a38] transition-colors ${ps.score != null ? 'hover:bg-[#1e2230] cursor-pointer' : ''}`}>
                          <td className="px-4 py-3 font-medium text-[#eef0f6]">{ps.label ?? '—'}</td>
                          ...
                          <td className="px-4 py-3 text-right text-[#eef0f6]">{ps.violationCount ?? '—'}</td>
