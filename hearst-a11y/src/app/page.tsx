...
import { countIssueTypes, countOccurrences, getSeverityCounts, isWcagPattern } from '@/lib/metrics'
...
async function getData(division?: string, allowedDivisions?: string[]) {
  ...
  const totalErrors = latestPatternsBySite.reduce((sum: number, patterns) =>
  ...
  return {
    ...
    scans,
    activeDivisions,
    stats: { totalPages, totalErrors, errorsResolved, siteCount: sites.length, totalIssueTypes },
    severityCounts,
    topViolations,
    scoreTrends,
    criticalSiteCount,
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E5E5EA', borderLeft: '4px solid #007AFF', padding: '20px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Component Issues</div>
            <div style={{ fontSize: 40, fontWeight: 800, color: '#007AFF', lineHeight: 1, letterSpacing: '-0.02em' }}>{stats.totalErrors}</div>
            <div style={{ fontSize: 12, color: '#57575A', marginTop: 8 }}>component instances with WCAG errors</div>
          </div>
          ...
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9F9FB', borderBottom: '1px solid #E5E5EA' }}>
                ...
                <th style={{ textAlign: 'right', padding: '10px 16px', fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Component Issues</th>
