...
export default function CostsPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0', color: '#57575A', fontSize: 14 }}>
      Loading…
    </div>
  )
  if (!data) return null
  ...
  return (
    <div style={{ padding: '24px 32px', background: '#F5F5F7', minHeight: '100vh' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#57575A', marginTop: 4, marginBottom: 0 }}>Claude API usage across all scans</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        <div style={statStyle}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Claude calls</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#1D1D1F', letterSpacing: '-0.02em' }}>{Number(totals.total_calls).toLocaleString()}</div>
          <div style={{ fontSize: 12, color: '#57575A', marginTop: 2 }}>all time</div>
        </div>
        <div style={statStyle}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Scans completed</div>
        ...
        <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #F5F5F7' }}>
              <th style={{ textAlign: 'left', padding: '10px 20px', fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              ...
              <th style={{ textAlign: 'right', padding: '10px 16px', fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Spend</th>
              <th style={{ textAlign: 'right', padding: '10px 16px', fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Calls</th>
              <th style={{ textAlign: 'right', padding: '10px 20px', fontSize: 11, fontWeight: 600, color: '#57575A', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Scans</th>
            </tr>
          </thead>
          <tbody>
            {(tab === 'division' ? byDivision : bySite).map((row: any, i: number, arr: any[]) => (
              <tr key={i} style={{ borderBottom: i < arr.length - 1 ? '1px solid #F5F5F7' : 'none' }}>
                <td style={{ padding: '11px 20px', fontWeight: 500, color: '#1D1D1F' }}>
                  {tab === 'site' ? (row.site_name ?? <span style={{ color: '#57575A' }}>Unknown</span>) : row.division}
                </td>
                {tab === 'site' && (
